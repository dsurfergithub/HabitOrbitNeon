...
// Returns per-DOW avg habits done per day of that weekday (last 12 weeks)
function dayOfWeekStats(log,habits){
  const doneSum=Array(7).fill(0),occurrences=Array(7).fill(0)
  ...
  for(let i=1;i<=84;i++){
    ...
    occurrences[dow]++
    habits.filter(h=>h.active).forEach(h=>{
      if(getStatus(log,h.id,ds)==='done') doneSum[dow]++
    })
  }
  const max=habits.filter(h=>h.active).length
  return doneSum.map((sum,i)=>({
    avg: occurrences[i]>0?Math.round(sum/occurrences[i]*10)/10:0,
    max,
    weeks: occurrences[i],
  }))
}

// Count days done in last 365 days for one habit
function habitDoneCount365(log,hid){
  let done=0;const d=new Date(TODAY)
  for(let i=0;i<365;i++){
    if(getStatus(log,hid,d.toISOString().split('T')[0])==='done') done++
    d.setDate(d.getDate()-1)
  }
...
function TabOrbita({S,setS,showToast,isMobile}){
  ...
  // 90d on mobile by default (13 cols → cells fit at 14px); 365d on desktop or when toggled
  const DAYS_BACK = (showFull||!isMobile) ? 364 : 89
  const CELL = isMobile ? (showFull ? 10 : 14) : 13
  const GAP  = isMobile ? (showFull ? 2 : 3) : 3

  const start=new Date(TODAY);start.setDate(start.getDate()-DAYS_BACK)
  ...
  return(
    <div style={{paddingTop:8}}>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:5,marginBottom:12}}>
        {activeHabits.map(h=>(
          <button key={h.id} onClick={()=>setS(p=>({...p,selectedHabit:h.id}))} style={{padding:'7px 12px',borderRadius:3,cursor:'pointer',fontSize:13,fontFamily:"'Share Tech Mono',monospace",border:`1px solid ${h.id===selId?'var(--neon)':'var(--border2)'}`,color:h.id===selId?'var(--neon)':'var(--text3)',background:h.id===selId?'var(--neon-dim)':'none',minHeight:38,boxShadow:h.id===selId?'var(--neon-glow-sm)':'none'}}>
            {h.emoji} {h.name}
          </button>
        ))}
      </div>

      {/* Legend + 90d/365d toggle */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:8,marginBottom:10}}>
        <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          {[{col:'var(--neon)',lbl:'Logrado',glow:true},{col:'var(--rest)',lbl:'Descanso'},{col:'var(--danger)',lbl:'Fallido'},{col:'var(--warn)',lbl:'Skip'},{col:'var(--surface2)',lbl:'Pendiente',border:'1px solid var(--border)'}].map(({col,lbl,glow,border})=>(
            <div key={lbl} style={{display:'flex',alignItems:'center',gap:4,fontFamily:"'Share Tech Mono',monospace",fontSize:11,color:'var(--text3)'}}>
              <div style={{width:10,height:10,borderRadius:2,background:col,boxShadow:glow?'0 0 3px var(--neon)':'none',border:border||'none',flexShrink:0}}/>
              {lbl}
            </div>
          ))}
        </div>
        {isMobile&&(
          <button onClick={()=>setShowFull(v=>!v)} style={{...SS.btnGhost,fontSize:8,padding:'6px 12px',minHeight:34,letterSpacing:1,borderColor:'var(--neon)',color:'var(--neon)'}}>
            {showFull?'90 DÍAS':'AÑO COMPLETO'}
          </button>
        )}
      </div>
      ...
      <div style={{overflowX:'auto',paddingBottom:6,marginBottom:14,WebkitOverflowScrolling:'touch'}}>
        <div style={{display:'flex',gap:GAP,marginBottom:4,paddingLeft:isMobile?0:22}}>
          {monthLabels.map((lbl,ci)=>(
            <span key={ci} style={{width:CELL,flexShrink:0,fontFamily:"'Share Tech Mono',monospace",fontSize:8,color:lbl?'var(--text2)':'transparent',lineHeight:'10px'}}>
              {lbl||'.'}
            </span>
          ))}
        </div>
            ...
            <div key={ci} style={{display:'flex',flexDirection:'column',gap:GAP}}>
              {c.map((d,di)=>{
                ...
                return<div key={di}
                  ...
                  style={{width:CELL,height:CELL,borderRadius:CELL>=13?3:2,cursor:'pointer',background:bg,
                    boxShadow:d.status==='done'?'0 0 4px var(--neon)':'none',
                    border:d.status==='pending'?'1px solid var(--border)':'none',
                    outline:d.isToday?'2px solid var(--text)':'none',
                    flexShrink:0,
                  }}
                />
              })}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:14}}>
        {[
          {val:doneCount,lbl:showFull||!isMobile?'LOGRADOS 365D':'LOGRADOS 90D'},
          {val:streak+'d',lbl:'RACHA ACTUAL'},
          {val:mStr+'d',lbl:'RACHA MÁXIMA'},
        ].map(({val,lbl})=>(
...
function TabStats({S,c30,isMobile}){
  ...
  const dowStats=useMemo(()=>dayOfWeekStats(S.log,S.habits),[S.log,S.habits])
  const taskHistory=Object.values(S.tasksHistory||{})
  const taskChecked=taskHistory.filter(d=>d.status==='checked').length
  const maxAvg=Math.max(...dowStats.map(d=>d.avg),1)
  ...
  return(
    <div style={{paddingTop:8}}>
      ...
      <SL>PATRÓN SEMANAL <span style={{fontSize:9,color:'var(--text3)',fontFamily:"'Share Tech Mono',monospace",letterSpacing:0,textTransform:'none',marginLeft:2}}>12 semanas</span></SL>
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:isMobile?3:4,marginBottom:16}}>
        {DOW_LABELS.map((l,i)=>{
          const {avg,max}=dowStats[i]
          const ratio=max>0?avg/max:0
          const col=ratio>=0.75?'var(--neon)':ratio>=0.45?'var(--cyan)':'var(--danger)'
          const barH=Math.max(3,Math.round((avg/maxAvg)*36))
          return(
            <div key={l} style={{padding:isMobile?'8px 2px':'10px 4px',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--r-sm)',textAlign:'center'}}>
              <div style={{fontFamily:"'Orbitron',monospace",fontSize:8,letterSpacing:0.5,color:'var(--text3)',marginBottom:5}}>{l}</div>
              <div style={{height:36,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
                <div style={{width:'68%',height:barH,background:col,boxShadow:ratio>=0.75?`0 0 5px ${col}`:'none',borderRadius:'2px 2px 0 0'}}/>
              </div>
              <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:isMobile?11:12,color:col,marginTop:4,fontWeight:600,lineHeight:1}}>{avg}</div>
              <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:8,color:'var(--text3)',marginTop:2}}>/{max}</div>
            </div>
          )
        })}
      </div>

      <SL>POR HÁBITO <span style={{fontSize:9,color:'var(--text3)',fontFamily:"'Share Tech Mono',monospace",letterSpacing:0,textTransform:'none',marginLeft:2}}>365 días</span></SL>
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--r-sm)',overflow:'hidden'}}>
        {activeHabits.map((h,i)=>{
          const streak=currentStreak(S.log,h.id)
          const done365=habitDoneCount365(S.log,h.id)
          const barPct=Math.round(done365/365*100)
          return(
            <div key={h.id} style={{padding:'12px 14px',borderBottom:i<activeHabits.length-1?'1px solid var(--border)':'none'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:7}}>
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <span style={{fontSize:14,color:'var(--text)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:isMobile?100:180}}>{h.name}</span>
                </div>
                <div style={{display:'flex',alignItems:'baseline',gap:5,flexShrink:0}}>
                  <span style={{fontFamily:"'Orbitron',monospace",fontSize:16,fontWeight:900,color:'var(--neon)',textShadow:'var(--neon-glow-sm)'}}>{done365}</span>
                  <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:'var(--text3)'}}> / 365d</span>
                  <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:'var(--text2)',marginLeft:4}}>{streak}d</span>
                </div>
              </div>
              <div style={{background:'var(--surface3)',height:3,borderRadius:2}}>
                <div style={{width:barPct+'%',height:'100%',background:'var(--neon)',boxShadow:'0 0 3px var(--neon)',borderRadius:2}}/>
              </div>
            </div>
          )
        })}
